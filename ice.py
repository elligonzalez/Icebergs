...
def area_PixelImageDimensions(mask,img):
    ...
    area = cv2.countNonZero(mask)*(300*300)/(width*height) #count number of pixels in processed images, multiply by image dimensions (300x300(km)), and divide by pixel width and height
...
def ice_olate(directory,layerName=None,display=False,setThresh=None,areaMethod=None,areaThresh=None,cutOff=None,retCnt = False):
    ...
    #Return
    if retCnt == True:
        return trackAreas,cnt,coords #return areas, pixel contours, and coordinates
    else:
